...
class ImageManager:
    ...
    async def run_in_executor(loop, executor, func, targeted_object, *args, **kwargs):
        ...
        return await loop.run_in_executor(executor, func, targeted_object, *args)

    def execute_funcs(self, function: Callable, args_list: List[Tuple[int, list, dict]]):
        if self.use_async:
    ...
    async def _execute_funcs_async(self, function: Callable, args_list: List[Tuple[int, list, dict]]):
