...
class Image:
    ...
    def _save_image(self, source_path, img_data, new_name=None):
    ...
    def _convert_image_format(self, source_path, new_name):
        ...
        with PIL_Image.open(str(source_path)) as img:
            img.save(new_file_path)
        os.remove(source_path) if source_path != new_file_path else print("Skipping deleting ...")
        return new_file_path
        
    def convert_image_format(self, base_location, original_name, original_format=None, new_name=None, target_format='png'):
        source_path = os.path.join(base_location, f"{original_name}.{original_format}" if original_format else original_name)
        if source_path.split(".")[-1] == "svg":
            print("SVG format is not supported.")
            return None
    ...
    def base64(self, path, new_name, img_format):
        img_data = base64.b64decode(self.data.split(',')[1])
    ...
    def download_logo_image(self, img_url, new_name, img_format):
        try:
            ...
            if format_match:
                ...
                return self.download_image(img_url, new_name, img_format)
        except:
            return
            
    def download_image(self, img_url=None, new_name=None, img_format=None):
        try:
            ...
            if img_name.strip():
                ...
                self._save_image(os.path.join(self.cache, f"{name}.{extension}"), img_data, new_name=None)
                return True
            else:
                return False
