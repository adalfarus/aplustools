...
def test_shared_struct_reference_and_restore() -> None:
    ...
    try:
        ...
        ss2 = SharedStruct(ref.struct_format, shm_name=ref.shm_name, overwrite_mp_lock=ref.lock)
        result = ss2.get_data()
...
@pytest.mark.parametrize(
    "executor_cls", [
        HyperScalingDynamicThreadPoolExecutor, LazyDynamicThreadPoolExecutor, LazySelfManagingDynamicThreadPoolExecutor,
    ],
)
def test_executor_submit_and_shutdown(executor_cls: _ty.Type[HyperScalingDynamicThreadPoolExecutor]) -> None:
...
def test_executor_map(executor_cls: _ty.Type[HyperScalingDynamicThreadPoolExecutor]) -> None:
...
def test_executor_shutdown_blocks_future_submit(executor_cls: _ty.Type[HyperScalingDynamicThreadPoolExecutor]) -> None:
