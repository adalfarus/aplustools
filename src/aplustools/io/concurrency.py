...
class GrowingThreadPoolExecutor:
    """
    A dynamic thread pool executor that expands or contracts based on workload.
    """

    def __init__(self, min_workers: int = 2, max_workers: int = 10, idle_time: int = 5) -> None:
        """
        Initialize the thread pool with dynamic sizing.
        :param min_workers: Minimum number of threads to keep alive.
        :param max_workers: Maximum number of threads allowed.
        :param idle_time: Idle time (seconds) before reducing the pool size.
        """
        self.min_workers = min_workers
        self.max_workers = max_workers
        self.idle_time = idle_time
        self._pool = _ThreadPoolExecutor(max_workers=min_workers)
        self._lock = _Lock()  # Lock to protect thread pool expansion
        self._task_count = 0  # Tracks how many tasks are currently being processed
        self._check_timer: _TimidTimer = _TimidTimer(start_now=False)
        self._check_timer.fire(idle_time, self._contract_if_needed, daemon=True)

    def submit(self, fn: _a.Callable, *args, **kwargs) -> _Future:
        """
        Submit a task to the thread pool.
        :param fn: The function to execute in the pool.
        """
        with self._lock:
            self._task_count += 1
            self._expand_if_needed()
        future = self._pool.submit(self._run_and_track, fn, *args, **kwargs)
        return future

    def _run_and_track(self, fn: _a.Callable, *args, **kwargs) -> None:
        """
        Wrapper around the function to track the task count and contract the pool after task completion.
        """
        try:
    ...
    def _expand_if_needed(self) -> None:
        """
        Dynamically expand the thread pool size if needed, up to the maximum size.
        """
        current_threads = len(self._pool._threads)
        if self._task_count > current_threads:
            self._pool._max_workers = min(self.max_workers, current_threads + 1)

    def _contract_if_needed(self) -> None:
        """
        Dynamically contract the thread pool size if idle threads exist and tasks are low.
        """
        with self._lock:
            if self._task_count < self._pool._max_workers:
                self._pool._max_workers = max(self.min_workers, self._pool._max_workers - 1)
