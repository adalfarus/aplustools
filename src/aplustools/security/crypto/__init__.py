"""TBA"""
raise NotImplementedError("This module is not done yet, please wait until aplustools[all]>=2.0.0.0b3")
from importlib import import_module as _import_module
import warnings as _warnings
import os as _os

# from ._keys import (Kyber, Argon2, _KYBER_KEYPAIR, _DILITHIUM_KEYPAIR, KKDF as _KKDF)   # TODO: Move to fallbacks
from .algos import Sym as _Sym, ASym as _ASym
from ._hashes import (HashAlgorithm as _HashAlgorithm, algorithm_names as _algorithm_names,
                      algorithm_ids as _algorithm_ids)
from ._algorithms import (KeyDerivationFunction as _KeyDerivation,
...
# __deps__: list[str] = ["bcrypt"]   # TODO: Move to fallbacks
__hard_deps__: list[str] = []
_enforce_hard_deps(__hard_deps__, __name__)


# _bcrypt = _optional_import("bcrypt")   # TODO: Move to fallbacks
...
_IMPL_KEYS: _ts.ModuleType | None = None
_IMPL_OTHER: _ts.ModuleType | None = None

def set_backend(new_backend: Backend) -> None:
    """Sets a new backend for the crypt submodule"""
    global BACKEND, _ISSUED_KEYS, _IMPL_KEYS, _IMPL_OTHER
    if new_backend not in {Backend.cryptography, Backend.pycryptodomex}:
        raise _NotSupportedError(f"{new_backend.info} is not supported by CoreCrypt")

    with _suppress_warnings():
        BACKEND = backend
        crypto_module = _import_module(str(backend.value))
        fallback_module = _import_module("aplustools.security.crypto._fallbacks")
        _IMPL_OTHER = crypto_module.hashes
        _IMPL_KEYS = crypto_module.keys

        # Dynamically set other methods
        for y in []:
            ...
        ...
        for sym_to_set in (_Sym.Cipher.AES, _Sym.Cipher.ChaCha20, _Sym.Cipher.TripleDES, _Sym.Cipher.Blowfish,
            ...
            elif hasattr(fallback_module.keys, key):
                sym_to_set._KEY = getattr(fallback_module.keys, key)
            else:
        ...
        for asym_to_set in (_ASym.Cipher.RSA, _ASym.Cipher.DSA, _ASym.Cipher.ECC, _ASym.Cipher.KYBER,
            ...
            elif hasattr(fallback_module.keys, keypair):
                asym_to_set._KEY = getattr(fallback_module.keys, keypair)
            else:
...
class CoreCrypt:
    ...
    def set_backend(self, backend: Backend) -> None:
        ...
        if backend not in {Backend.cryptography, Backend.pycryptodomex}:
            raise _NotSupportedError(f"{backend.info} is not supported by CoreCrypt")
...
class DataEncryptor:
    ...
    def __init__(self, password_key_or_key_size: _ty.Literal[128, 192, 256] | bytes | tuple[_ty.Literal[128, 192, 256], str] = 256,
        ...
        self._crypt: CoreCrypt = CoreCrypt(backend=backend)
        self._key: _BASIC_KEYTYPE = _Sym.Cipher.AES.key(password_key_or_key_size)
